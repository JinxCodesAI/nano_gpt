...
def simulate_orchestrator():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        logger = TrainingLogger(log_dir=temp_dir, enabled=True)
        sample_config = {
            'learning_rate': 6e-4,
            'batch_size': 12,
            'gradient_accumulation_steps': 5,
            'scaling_schedule_file': config_file
        }
        logger.setup(sample_config)
        print(f"Logging to: {logger.log_file_path}\n")
            ...
            if scaling_schedule:
                ...
                if loss_triggered or timeout_triggered:
                    ...
                    logger.log_operation_start(
                        iter_num * 10, next_op['name'], next_op['value'], trigger_reason,
                        val_loss, next_op['trigger_loss'], next_op['max_wait_iters']
                    )
                    ...
                    if op_name == 'change_lr':
                        ...
                        logger.log_operation_success(iter_num * 10, op_name, {
                            'old_lr_multiplier': old_lr,
                        ...
                        logger.log_operation_success(iter_num * 10, op_name, {
                            ...
                            'new_grad_accum_steps': gradient_accumulation_steps,
                            'old_multiplier': old_multiplier,
                        ...
                        logger.log_operation_success(iter_num * 10, op_name, {
                            'old_lr_schedule_offset': old_offset,
        ...
        # Close logger and show log content
        logger.close()
        ...
        # Show only operation-related lines for brevity
        operation_lines = [line for line in log_content.split('\n') if 'OPERATION_' in line]
        for line in operation_lines:
            print(line)

        print(f"\nTotal operation log entries: {len(operation_lines)}")
        print("\n✅ Manual test with logging completed successfully!")
        return True
