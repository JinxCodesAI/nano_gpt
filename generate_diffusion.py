...
# -----------------------------------------------------------------------------
out_dir = 'out-diffusion' # ignored if init_from is not 'resume'
...
if init_from == 'resume':
    ...
    ckpt_path = os.path.join(out_dir, 'ckpt_5000_d.pt')
...
with torch.no_grad():
    with ctx:
        for k in range(num_samples):
            y = model.generate_diffusion(start_ids[k:k+1], max_steps, temperature=temperature, top_k=top_k)
