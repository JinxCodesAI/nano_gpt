...
4. **Remasking strategies.** Inside `apply_remasking_step`:
   - **Critic-guided remasking:** If the main model exposes `add_critic_head`, the script queries `model.critic_scores()` to prioritize tokens with high predicted error probability.【F:sample.py†L384-L387】【F:sample_utils.py†L468-L511】
   - **Intelligent self-remasking:** Uses the diffusion logits to compute per-token uncertainty (1 – probability of the sampled token) and masks the highest-uncertainty positions.【F:sample_utils.py†L513-L562】
   - **Random fallback:** Selects a random subset when no guidance is available.【F:sample_utils.py†L564-L579】
   Threshold mode can replace ratio-based selection, masking all tokens whose wrongness exceeds a schedule-driven threshold. Each path signals early termination when nothing qualifies for remasking, letting the main loop exit before exhausting `iterations`.【F:sample_utils.py†L474-L579】
