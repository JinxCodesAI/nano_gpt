...
weight_unmask_task_min = 0.1    # Initial weight for the "fill-in-the-blank" task (emphasize early)
weight_unmask_task_max = 1.0    # Final weight for the "fill-in-the-blank" task (balance later)
weight_remask_task_min = 1    # Initial weight for the "proofreading" task (lower priority early)  
weight_remask_task_max = 1    # Final weight for the "proofreading" task (emphasize later)

# Curriculum learning settings
penalty_mask_correct = 0.5      # Final discount for wrongly masking a correct token
masking_warmup_iters = 3000     # Iterations to ramp up the penalty_mask_correct
proofreading_warmup_iters = 1000  # Iterations to ramp up the "re-masking" task
entropy_penalty = 0.1
